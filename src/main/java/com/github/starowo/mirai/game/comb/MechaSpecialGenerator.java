...
public class MechaSpecialGenerator extends ImageGenerator {
    ...
    public BufferedImage drawBoard(NumberHive.Board board, GamePlayer player, int index) throws IOException {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            gb.drawImage(iPiece, 64, 128 + 15 + 64 * i, null);
        }
        for (int i = 3; i < 7; i++) {
            if (board.board[i] == null)
                continue;
            int[] directions = board.board[i].directions;
            BufferedImage iPiece = getCardImg(dir, directions[0], directions[1], directions[2]);
            gb.drawImage(iPiece, 128, 96 + 15 + 64 * (i - 3), null);
        }
        for (int i = 7; i < 12; i++) {
            if (board.board[i] == null)
                continue;
            int[] directions = board.board[i].directions;
            BufferedImage iPiece = getCardImg(dir, directions[0], directions[1], directions[2]);
            gb.drawImage(iPiece, 192, 64 + 15 + 64 * (i - 7), null);
        }
        for (int i = 12; i < 16; i++) {
            if (board.board[i] == null)
                continue;
            int[] directions = board.board[i].directions;
            BufferedImage iPiece = getCardImg(dir, directions[0], directions[1], directions[2]);
            gb.drawImage(iPiece, 256, 96 + 15 + 64 * (i - 12), null);
        }
        for (int i = 16; i < 19; i++) {
            if (board.board[i] == null)
                continue;
            int[] directions = board.board[i].directions;
            BufferedImage iPiece = getCardImg(dir, directions[0], directions[1], directions[2]);
            gb.drawImage(iPiece, 320, 128 + 15 + 64 * (i - 16), null);
        }
        gb.dispose();
