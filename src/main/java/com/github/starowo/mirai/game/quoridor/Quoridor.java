...
public class Quoridor implements IGame {
    ...
    public int[] walls = new int[]{10, 10};

    public int[] timecard = new int[]{3, 3};

    public ArrayList<Character> directions = Lists.newArrayList('上', '右', '下', '左');
    public ArrayList<Character> rows = Lists.newArrayList('i', 'j', 'k', 'l', 'm', 'n', 'o', 'p');
    public ArrayList<Character> cols = Lists.newArrayList('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h');
    Thread thread = new ThreadTime(this);
    public int time = 90;

    public Quoridor(Group group) {
        this.group = group;
    }
    ...
    public void start() {
        ...
        try {
            ...
            // group.sendMessage("游戏开始，" + (status == 1 ? player1.name : player2.name) + "先手，请选择一个位置(1-9)放置角色"); // old: place tile first
            group.sendMessage("游戏开始，" + (status == 1 ? player1.name : player2.name) + "先手，请移动或放置障碍");

            // new: default tile placement
            tiles[0] = c2i(0, 8);
            tiles[1] = c2i(16, 8);
    ...
    public Message input(GamePlayer player, MessageChain msg, boolean isGroup, boolean at) {
        ...
        try {
            ...
            if (tiles[p] == -1) {
                try {
                    int pos = Integer.parseInt(String.valueOf(in));
                    if (pos < 1 || pos > 9) {
                        throw new NumberFormatException();
                    }
                    tiles[p] = c2i(p == 0 ? 0 : 16, 2 * (pos - 1));
            ...
            try {
                ...
                int n2 = num % 10 - 1;
                if (n1 > 8 || n1 < 0) {
                    return new MessageChainBuilder().append("输入错误：").append(text.substring(1)).append(" 数字超界；\n请按照如【b34】的格式输入，字母在前，数字在后").build();
                }
                if (Math.abs(n1 - n2) != 1) {
                    return new MessageChainBuilder().append("输入错误：").append(text.substring(1)).append(" 数字不相邻").build();
                }
                if (tiles[0] == tiles[1]) {
                ...
                if (!validBoard(tiles[0], 16, board, new HashSet<>()) || !validBoard(tiles[1], 0, board, new HashSet<>())) {
                    if (col) {
                ...
                if (at)
                    return new MessageChainBuilder().append("输入错误：").append(text.substring(1)).append(" 不是有效的数字；\n请按照如【b34】的格式输入，字母在前，数字在后").build();
                else
                    return null;
            }
    ...
    private void sendImage() {
        round++;
        BufferedImage iBlank = new BufferedImage(8*60+48*3+1, 8*60+48*3+1, 2);
        Graphics2D g = iBlank.createGraphics();
        g.setColor(Color.white);
        g.fillRect(0, 0, 8*60+48*3+1, 8*60+48*3+1);
        Font font = new Font("微软雅黑", Font.BOLD, 24);
        g.setFont(font);
        g.setColor(Color.BLACK);
        char c = 'a';
        for (int i = 1; i <= 9; i++) {
            int w = g.getFontMetrics().stringWidth(String.valueOf(i));
            int h = g.getFontMetrics().getHeight();
            int centerX = 48 * i + (48 - w) / 2 + 12 * (i - 1);
            int centerY = (48 - h) / 2 + g.getFontMetrics().getAscent();
            g.drawString(String.valueOf(i), centerX, centerY);
        }
        for (int i = 1; i <= 9; i++) {
            int w = g.getFontMetrics().stringWidth(String.valueOf(i));
            int h = g.getFontMetrics().getHeight();
        ...
        for (int i = 1; i <= 8; i++, c++) {
            ...
            int centerY = 8*60+48*2 + ((48 - h) / 2 + g.getFontMetrics().getAscent());
            g.drawString(String.valueOf(c), centerX, centerY);
        }
        for (int i = 1; i <= 8; i++, c++) {
            int w = g.getFontMetrics().charWidth(c);
            int h = g.getFontMetrics().getHeight();
            int centerX = 8*60+48*2 + ((48 - w) / 2);
    ...
    private boolean check() {
        return i2c(tiles[0])[0] == 16 || i2c(tiles[1])[0] == 0;
    }

    private static int[] i2c(int i) {
        return new int[]{i % 17, i / 17};
    }
    ...
    public static boolean canMove(int pos, int direction, int[][] board) {
        ...
        return x >= 0 && x < 17 && y >= 0 && y < 17 && board[y][x] == 0;
    }
    ...
    public void stop() {
        ...
        int exp_win = Math.min(Math.max(0, (round - 15) * 100), 2000);
    ...
    public static class ThreadTime extends Thread {
        ...
        public ThreadTime(Quoridor game) {
            this.game = game;
        }
